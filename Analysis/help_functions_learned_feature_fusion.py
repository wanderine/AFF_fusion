...
def Lff_image_feat_extract_model(model_path='image_model_patients_extended_alt_lr/fold_4'):
...
def scheduler_Lff(epoch, lr):
    return lr

def build_Lff_model(lr=1e-5, pat=50, im_model_weights_path = 'image_model_patients_extended_alt_lr/fold_4', num_reg_param = 7):
    image_feature_model = Lff_image_feat_extract_model(im_model_weights_path)

    image_input = keras.Input(shape=(224, 224, 3))
    register_input = keras.Input(shape=(num_reg_param,))

    image_features = image_feature_model(image_input, training=False)
    image_norm_layer = LayerNormalization(axis=-1, scale = False, center = False, name='img_norm')
    image_features = image_norm_layer(image_features)

    image_output = Dense(512, activation='relu', name='image_dense1')(image_features)
    image_output = Dropout(0.25, name='img_dropout_1')(image_output)
    image_output = Dense(256, activation='relu', name='image_dense2')(image_output)
    image_output = Dropout(0.25, name='img_dropout_2')(image_output)
    image_output = Dense(128, activation='relu', name='image_dense3')(image_output)

    reg_norm_layer = LayerNormalization(axis=-1, scale = False, center = False, name='reg_norm')
    register_input_norm = reg_norm_layer(register_input)

    register_output = Dense(512, activation='relu', name='register_dense1')(register_input_norm)
    register_output = Dropout(0.25, name='reg_dropout_1')(register_output)
    register_output = Dense(256, activation ='relu', name='register_dense2')(register_output)
    register_output = Dropout(0.25, name='reg_dropout_2')(register_output)
    register_output = Dense(128, activation = 'relu', name = 'register_dense3')(register_output)

    combined = concatenate([image_output, register_output])

    output = Dropout(0.25, name='combined_dropout1')(combined)
    output = Dense(128, activation='relu', name='combined_dense1')(output)
    output = Dropout(0.25, name='combined_dropout2')(output)
    output = Dense(64, activation='relu', name='combined_dense2')(output)
    output = Dropout(0.25, name='combined_dropout3')(output)
    output = Dense(1, activation='sigmoid', name='combined_dense3')(output)
    ...
    callback_lr = LearningRateScheduler(scheduler_Lff, verbose = 1)

    callback = [callback_metric, callback_lr]

    return model, callback, optimizer

def Lff_train(model, X_train, Y_train, X_valid, Y_valid, callback, class_weights, optimizer, num_epochs):
...
def crossvalidation_LFF_model(n_folds=5, learning_rate=1e-4, patience=50, epochs=1000, save_csv=False, model_num=1, folder='csv_learned_feature_fusion', variable_selection_func = load_registerdata_7_nonbinary_fold, only_age_sex = False, all_register_parameters = False, train_metrics_path='Plots_for_learned_feature_fusion_2_param', save_model_path='learned_feature_fusion_2_param_saved_models', repetition=1):
    ...
    for i in range(0, len(registerdata_lst_np)):
        ...
        Lff_model, callback, optimizer = build_Lff_model(lr=learning_rate, pat=patience, im_model_weights_path='image_model_weights/fold_' + str(i) + '.h5', num_reg_param = num_reg_param)

        history = Lff_train(Lff_model, X_train=[im_train, registerdata_train], Y_train=labels_train, X_valid=[im_valid, registerdata_valid], Y_valid=labels_valid, callback=callback, class_weights=class_weights, optimizer=optimizer, num_epochs=epochs)

        Y_pred = Lff_model.predict([im_test, registerdata_test])

        new_labels_pred, new_patient_IDs, new_labels_test = pred_patient(Y_pred, patient_IDs_test, labels_test)

        mean_tpr, mean_fpr, lst_acc, lst_sens, lst_spec, lst_mcc = Evaluation_CrossVal_patients(history, new_labels_test, new_labels_pred, Lff_model, save_csv, i, model_num, mean_tpr, mean_fpr, lst_acc, lst_sens, lst_spec, lst_mcc, folder=folder, repetition=repetition)
        ...
        Lff_model.save(save_model_path + '/fold_' + str(i+1) + '.h5')
